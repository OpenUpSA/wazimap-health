...
POPULATION_GROUP_ORDER = ('Black African', 'Coloured', 'Indian or Asian',
                          'White', 'Other')
POPULATION_GROUP_ORDER_2016 = ('Black African', 'Coloured', 'Indian or Asian',
                               'White')
GENDER_ORDER = ('Female', 'Male')
PROVINCE_ORDER = ('Eastern Cape', 'Free State', 'Gauteng', 'KwaZulu-Natal',
                  'Limpopo', 'Mpumalanga', 'North West', 'Northern Cape',
                  'Western Cape', 'Outside South Africa', 'Unspecified')
REGION_ORDER = ('South Africa', 'SADC', 'Rest of Africa', 'Other',
                'Unspecified')
CITIZENSHIP_ORDER = ('Yes', 'No', 'Unspecified')
EDUCATION_LEVEL_KEY_ORDER = ('Less than Grade9', 'Grade 9', 'Grade 10/11',
                             'Matric/matric equivalent', 'Any tertiary')
EMPLOYMENT_STATUS_KEY_ORDER = ('Employed', 'Unemployed',
                               'Discouraged work-seeker',
                               'Other not economically active')
EMPLOYMENT_SECTOR_KEY_ORDER = ('Formal sector', 'Informal sector',
                               'Private household', 'Do not know')
EMP_EDU_TRAIN_KEY_ORDER = ('NEET', 'School/post-school', 'Employed')
HH_EMPLOYMENT_KEY_ORDER = ('No employed adult', 'At least one employed adult')
ELECTRICITY_ACCESS_KEY_ORDER = ('No electricity',
                                'Have electricity for some things',
                                'Have electricity for everything')
TOILET_ACCESS_KEY_ORDER = ('No toilet facilities', 'Flush toilet',
                           'Pit latrine-ventilated', 'Chemical toilet',
                           'Unventilated pit latrine/Bucket toilet', 'Other')
WATER_ACCESS_KEY_ORDER = ('No piped water', 'On site', '< 1km', '> 1km')
TYPE_OF_DWELLING_ORDER = ('Formal', 'Informal not in backyard',
                          'Informal in backyard', 'Traditional', 'Other')
INTERNET_ACCESS_ORDER = ('From home', 'From cell phone', 'From work',
                         'From elsewhere', 'No access to internet')
TYPE_OF_AREA_ORDER = ('Formal residential', 'Informal residential',
                      'Traditional residential', 'Farms',
                      'Parks and recreation', 'Collective living quarters',
                      'Industrial', 'Small holdings', 'Vacant', 'Commercial')
TYPE_OF_AREA_ORDER_2016 = ('Farm areas', 'Tribal/Traditional areas',
                           'Urban areas')
HH_CROWDED_KEY_ORDER = ('Overcrowded', 'Non-overcrowded')
LIVING_WITH_PARENTS_KEY_ORDER = ('Both parents', 'Mother only', 'Father only',
                                 'Neither parent')
DIFFICULTY_FUNCTIONING_KEY_ORDER = ('Seeing, even when using eye glasses',
                                    'Hearing, even when using a hearing aid',
                                    'Communication', 'Walking', 'Remembering',
                                    'Self care')
GIVEN_BIRTH_KEY_ORDER = ('Given birth', 'Never given birth', 'Do not know',
                         'Unspecified')
...
def get_profile(geo, profile_name, request):
    ...
    try:
        ...
        display_profile = 'WC' if (geo.geo_code == 'WC' or
                                   'WC' in [cg.geo_code
...
def get_economic_opportunities_profile(geo,
                                       session,
                                       display_profile,
                                       comparative=False):
    ...
    youth_unemployment_by_definition = OrderedDict((('Official', {
        "name":
        "Official definition",
        "values": {
            "this": youth_labour_force_official['Unemployed']['values']['this']
        },
        "numerators": {
            "this":
            youth_labour_force_official['Unemployed']['numerators']['this']
        }
    }), ('Expanded', {
        "name": "Expanded definition",
        "values": {
            "this": youth_labour_force_expanded['Unemployed']['values']['this']
        },
        "numerators": {
            "this":
            youth_labour_force_expanded['Unemployed']['numerators']['this']
        }
    })))
    youth_unemployment_by_definition['metadata'] = youth_labour_force_official[
        'metadata']
    ...
    final_data = {
        'youth_official_unemployment': {
            "name":
            "Youth (aged 15-24) unemployment rate using the official definition *",
            "values": {
                "this":
                youth_labour_force_official['Unemployed']['values']['this'],
            }
        },
        'youth_unemployment_by_definition':
        youth_unemployment_by_definition,
        'youth_official_unemployment_by_gender':
        youth_official_unemployment_by_gender,
        'youth_employed': {
            "name": "Of youth are employed",
            "values": {
                "this": youth_employment_status['Employed']['values']['this'],
            }
        },
        'youth_employment_status':
        youth_employment_status,
        'pop_employment_status':
        pop_employment_status,
        'youth_formal_sector': {
            "name": "Youth in the formal sector",
            "values": {
                "this":
                youth_employment_sector['Formal sector']['values']['this'],
            }
        },
        'youth_employment_sector':
        youth_employment_sector,
        'pop_employment_sector':
        pop_employment_sector,
        'youth_neet': {
            "name":
            "Of youth are not in employment, education or training (NEET)",
            "values": {
                "this": youth_emp_edu_train_status['NEET']['values']['this']
            }
        },
        'youth_emp_edu_train_status':
        youth_emp_edu_train_status,
        'youth_neet_by_gender':
        youth_neet_by_gender,
        'youth_no_working_adults': {
            "name": "Of youth live in households without an employed adult",
            "values": {
                "this":
                youth_household_employment['No employed adult']['values'][
                    'this']
            }
        },
...
def get_safety_profile(geo, session, display_profile, comparative=False):
    ...
    if display_profile == 'WC':
        ...
        final_data = {
            ...
            youth_accused_by_pop_group_per_10k,
            'youth_victims_by_gender_per_10k':
            youth_victims_by_gender_per_10k,
            'youth_accused_by_gender_per_10k':
            youth_accused_by_gender_per_10k,
            'youth_victims_by_year':
            youth_victims_by_year,
            'youth_accused_by_year':
            youth_accused_by_year,
            ...
            'contact_crimes_by_year':
            contact_crimes_by_year,
            'property_crimes_by_year':
            property_crimes_by_year
        }
